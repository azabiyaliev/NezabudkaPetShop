...
const Carts: React.FC<Props> = ({ carts }) => {
  ...
  const productsToBuy: { price: number; amount: number }[] = carts.map(
    (product) => {
      if (product.product) {
        return {
    ...
    >
      <thead>
        <tr>
          ...
          ></th>
          <th style={{ width: "40%", textAlign: "center", fontSize: "16px" }}>
            Товар
          </th>
      ...
      <tfoot>
        <tr>
          <td scope="row"></td>
          <td scope="row" style={{ fontSize: "16px" }}>
            <b>Итого:</b>
          </td>
          <td scope="row"></td>
          <td scope="row" style={{ fontSize: "16px" }}>
          ...
          <td scope="row">
            <Typography
              level="h2"
              sx={{ fontSize: "lg", color: "rgba(250, 179, 1, 1)" }}
            >
