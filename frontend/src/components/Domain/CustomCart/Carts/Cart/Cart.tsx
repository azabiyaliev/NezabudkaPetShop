...
const Cart: React.FC<Props> = ({ productCart }) => {
  ...
  const removeQuantity = async (id: number) => {
    if (productCart.quantity > 1) {
      ...
      enqueueSnackbar(
        "Количество товара не может быть меньше 1! Если хотите убрать товар из списка, то можете удалить его из списка.",
        { variant: "warning" },
      );
    }
  };
  ...
  const totalCost: number =
    productCart.product.productPrice * productCart.quantity;
  ...
  return (
    <tr>
      ...
      <td>
        <Typography
          level="h2"
          sx={{ fontSize: "md", color: "rgba(250, 179, 1, 1)" }}
        >
