...
import { DeliveryMethod, OrderStatus, PaymentMethod, Role } from '@prisma/client';
...
export class SeedService {
  ...
  async seed() {
    // Очистка всех таблиц
    const tablesToClear = [
      'cartItem', 'cart', 'favorite', 'passwordReset', 'user',
      'orderItem', 'order', 'statistic', 'products', 'siteEdition',
      'photoByCarousel', 'brand', 'category', 'companyPages',
      'bonusProgramPage', 'deliveryPage'
    ];

    for (const table of tablesToClear) {
      // Используем индексный доступ с проверкой типа
      const model = this.prisma[table as keyof typeof this.prisma] as any;
    ...
    const userRoles: Role[] = ['superAdmin', 'admin', 'client'];
    const userDataArray = [
      {
    ...
    // Имена и фамилии для клиентов
    const firstNames = ['Михаил', 'Александр', 'Иван', 'Сергей', 'Дмитрий', 'Анна', 'Елена', 'Мария', 'Ольга', 'Кристина'];
    const lastNames = ['Смирнов', 'Иванов', 'Петров', 'Сидоров', 'Козлов', 'Морозова', 'Волкова', 'Кузнецова', 'Соколова', 'Попова'];
    ...
    // Фильтруем клиентов для дальнейшего использования
    ...
    // Создание фото для карусели
    const carouselPhotos = [
      { photo: '/fixtures/editionSitePhoto/photo1.jpg', link: 'https://example.com/photo1.jpg' },
      { photo: '/fixtures/editionSitePhoto/photo2.jpg', link: 'https://example.com/photo2.jpg' },
      { photo: '/fixtures/editionSitePhoto/photo2.jpg', link: 'https://example.com/photo3.jpg' },
      { photo: '/fixtures/editionSitePhoto/photo4.jpg', link: 'https://example.com/photo3.jpg' },
      { photo: '/fixtures/editionSitePhoto/photo5.jpg', link: 'https://example.com/photo3.jpg' }
    ];
    ...
    // Создание брендов
    const brandNames = [
      'AVZ', 'BAYER', 'Catchow', 'Dog-Chow', 'Flexi', 'Friskies', 'Gourmet', 
      'Happy Сat', 'Happy Dog', 'Purina Pro Plan', 'ROYAL CANIN', 'WOLF OF WILDERNESS',
      'James Wellbeloved', 'HILL\'S PET NUTRITION', 'Arden Grange', 'Applaws',
      'Burns', 'Whiskas', 'Concept for Life', 'Cosma', 'Purizon', 'Rosie\'s Farm',
      'Tigerino', 'Pedigree', 'SkogsFRO', 'Qushy', 'Мнямс', 'Brit', 'Jarvi', 'ANIMAL ISLAND'
    ];
    ...
    for (let i = 0; i < brandNames.length; i++) {
      const hasDescription = Math.random() > 0.3; // 70% брендов будут иметь описание
      const description = hasDescription 
        ? brandDescriptions[Math.floor(Math.random() * brandDescriptions.length)]
        : null;
      
      const logoIndex = Math.floor(Math.random() * 5) + 1;
      const logo = `/fixtures/Brands/${brandNames[i].toLowerCase().replace(/\s+/g, '-')}.${Math.random() > 0.5 ? 'png' : 'webp'}`;
    ...
    const mainCategories = [
      ...
      { title: 'Рептилии', image: '/fixtures/categoryIcons/lizard_transparent.png' },
    ...
    // Подкатегории для основных категорий
    const subCategoryData: Record<string, string[]> = {
      'Собаки': ['Сухой корм', 'Влажные корма', 'Шампуни и гели', 'Одежда для собак', 'Лежаки и подушки', 'Питание для щенков', 'Переноски'],
      'Кошки': ['Сухие корма', 'Влажные корм', 'Шампуни для кошек', 'Одежда для кошек', 'Груминг и уход', 'Игрушки для кошек', 'Когтеточки'],
      'Грызуны': ['Корма для грызунов', 'Клетки для грызунов', 'Игрушки для грызунов', 'Наполнитель для клеток', 'Лакомства для грызунов'],
      'Птицы': ['Корм для птиц', 'Клетки и вольеры', 'Игрушки для птиц', 'Витамины и добавки', 'Поилки и кормушки'],
      'Рыбки': ['Корма для рыб', 'Аквариумы и аксессуары', 'Фильтры для аквариумов', 'Освещение для аквариумов', 'Декорации для аквариумов'],
      'Рептилии': ['Корма для рептилий', 'Террариумы', 'Обогреватели для террариумов', 'Декорации для террариумов', 'Термометры и гигрометры для террариумов'],
      'Другие питомцы': ['Пищевые смеси', 'Миски и поилки', 'Палочки и витаминные добавки', 'Гнезда и укрытия', 'Минеральные добавки', 'Обогреватели для воды', 'Влажные и сухие смеси для кормления', 'Солевые растворы и добавки']
    };
    ...
    // Создание продуктов
    const productPrefixes = ['Про', 'Био', 'Натур', 'Эко', 'Люкс', 'Макси', 'Мини', 'Комфорт', 'Зоо', 'Ультра'];
    const productTypes = ['Стандарт', 'Премиум', 'Универсальный', 'Гипоаллергенный', 'Повседневный', 'Полнорационный'];
    const productPhotos = [
    ...
    const manufacturers = ['Испания', 'Германия', 'Франция', 'Италия', 'Швейцария', 'Россия', 'США', 'Япония', 'Корея', 'Великобритания'];
    const ages = ['Взрослый', 'Щенок/Котенок', 'Пожилой', 'Все возрасты'];
    const weights = [0.5, 1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20];

    const products = [];
    for (const subCategory of subCategories) {
      // Генерируем от 3 до 7 продуктов для каждой подкатегории
      const productsCount = Math.floor(Math.random() * 5) + 3;
      
      for (let i = 0; i < productsCount; i++) {
        const randomPrefix = productPrefixes[Math.floor(Math.random() * productPrefixes.length)];
        const randomType = productTypes[Math.floor(Math.random() * productTypes.length)];
        const randomBrand = brands[Math.floor(Math.random() * brands.length)];
        const randomPhoto = productPhotos[Math.floor(Math.random() * productPhotos.length)];
        const randomDescription = productDescriptions[Math.floor(Math.random() * productDescriptions.length)];
        ...
        if (onSale) {
          ...
          startDateSales.setDate(now.getDate() - Math.floor(Math.random() * 10));
          
          endDateSales = new Date(now);
          endDateSales.setDate(now.getDate() + Math.floor(Math.random() * 30) + 5);
        }
        ...
        // Добавляем даты акций только если продукт на акции
    ...
    // Создание заказов и их элементов
    const orderStatuses: OrderStatus[] = ['Pending', 'Confirmed', 'Shipped', 'Delivered', 'Canceled'];
    const paymentMethods: PaymentMethod[] = ['ByCard', 'ByCash'];
    const deliveryMethods: DeliveryMethod[] = ['Delivery', 'PickUp'];
    const cities = ['Бишкек', 'Ош', 'Кара-Балта', 'Талас', 'Нарын', 'Чолпон-Ата', 'Балыкчы', 'Джалал-Абад'];
    const streets = ['Ленина', 'Гагарина', 'Московская', 'Киевская', 'Абдрахманова', 'Фрунзе', 'Чуй', 'Боконбаева', 'Байтик Баатыра'];
    const orderComments = [
    ...
    for (let i = 0; i < ordersCount; i++) {
      ...
      const useBonus = Math.random() > 0.7; // 30% заказов используют бонусы
      const bonusUsed = useBonus ? Math.floor(Math.random() * 300) + 50 : undefined;
      
      const randomComment = orderComments[Math.floor(Math.random() * orderComments.length)];
      ...
      // Создаем заказ с его элементами
    ...
    await this.prisma.companyPages.create({
      data: {
        text: 'С 2008 года магазин зоотоваров "Незабудка" радует своих клиентов и их питомцев. Теперь Мы вышли в онлайн и готовы предложить один самых широких ассортиментов зоотоваров в Бишкеке на доставку.\n' +
    ...
    // Создаем страницу о бонусной программе
